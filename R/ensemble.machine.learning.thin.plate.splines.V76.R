...
machisplin.mltps<-function(int.values, covar.ras, tps=TRUE, smooth.outputs.only=FALSE, trouble=FALSE){
f <- list()
##TEMP OVERRIDE
n.cores=1
...
print("IMPORTANT: all data input are assummed to be WGS1984 projection/datum")
...
if(nrow(Mydata)<40){print(paste("Warning!",(nrow(Mydata)), " sites are too few points to use this method (this requires a minimum of 40 data points)."))}
...
proj4string(MyShp) <- CRS("+proj=longlat +datum=WGS84 +ellps=WGS84")
#######################
...
if(n.cores==1){
	...
	if(n.spln>1){iter.clim<-seq(1,n.spln)} else {iter.clim<-1}# length = number of climate variables
		for(i in iter.clim){
			...
			#train with 90% of data is <4000 or 10% if >4000 total rows, if < 30 samplesm, k =3, 
			if (nrow(dat_tps[[i]])>4000){
				nfolds <- 10
				kfolds <- machisplin.kfold(dat_tps[[i]], nfolds)
				bag.fraction.in<-0.5
				tree.complexity.in<-25
				}	 
			if (nrow(dat_tps[[i]])>100 & nrow(dat_tps[[i]])<4000){
				nfolds <- 10
				kfolds <- machisplin.kfold(dat_tps[[i]], nfolds)
				bag.fraction.in<-0.5
				tree.complexity.in<-25
				} 
			if (nrow(dat_tps[[i]])>60 & nrow(dat_tps[[i]])<101){
			    nfolds <-(as.integer(nrow(dat_tps[[i]])/10)-1)
				kfolds <- machisplin.kfold(dat_tps[[i]], nfolds)
				bag.fraction.in<-0.75
				tree.complexity.in<-10
				}
			if (nrow(dat_tps[[i]])>39 & nrow(dat_tps[[i]])<61){
			    nfolds <- 2
				kfolds <- machisplin.kfold(dat_tps[[i]], nfolds)
				bag.fraction.in<-1
				tree.complexity.in<-5
				}	
			if (nrow(dat_tps[[i]])<40){stop("too few input sites")}
			...
			print(paste("*******Iteration",(v),"(of", nfolds,")*************************************************"))
			...
			####train models
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Boosted Regresion Trees: Iteration",(v),"(of", nfolds,")"))
            print(paste("                                         ",Sys.time()))
			gc()
			mod.brt.tps.elev<- machisplin.gbm.step(data=train, gbm.x = 2:(n.covars+1), gbm.y =1, family = "gaussian", tree.complexity = tree.complexity.in, learning.rate = 0.01, bag.fraction = bag.fraction.in, plot.main = FALSE, silent = TRUE)
			...
			#RF
			gc()
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Random Forests: Iteration",(v),"(of", nfolds,")"))
			print(paste("                                         ",Sys.time()))
			...
			#NN
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Neural Networks: Iteration",(v),"(of", nfolds,")"))
			print(paste("                                         ",Sys.time()))
			gc()
			...
			#MAR
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Multivariate Adaptive Regression Splines: Iteration",(v),"(of", nfolds,")"))
			print(paste("                                         ",Sys.time()))
			gc()
			...
			#SVM
			gc()
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Support Vector Machines: Iteration",(v),"(of", nfolds,")"))
			print(paste("                                         ",Sys.time()))
			...
			#GAM
			gc()
			print(paste("Running k-fold cross-validation for ",(out.names[i]),": Generalized Additive Models: Iteration",(v),"(of", nfolds,")"))
			print(paste("                                         ",Sys.time()))
			...
			for(k in mods.run){	
			  ...
			  if (k=="b"){
				...
				mod.run="BRT"
				#run model with all points
				mod.brt.tps.FINAL<- machisplin.gbm.step(data=dat_tps[[i]], gbm.x = 2:(n.covars+1), gbm.y =1, family = "gaussian", tree.complexity = tree.complexity.in, learning.rate = 0.001, bag.fraction = bag.fraction.in, plot.main = FALSE, silent = TRUE)
				#store variable importance
			...
			if(tps==TRUE) {
				...
				#perform TPS on each grid and then mosaic
				if(nRx*nCx>1){
					Full.cords<-dat_tps[[i]][,c(n.covars,n.covars+1)]
					pred_TPS_elev<-NULL
					for (h in 1:(nRx*nCx)){
						gc()
				...
				if(nRx*nCx>1){
					...
					for (j in 1:nRx){
					  for (h in 1:nCx){
						...
						bR2 <- terra::resample(bR2, bR1, method = "near")
						##
...
#' 
#' #run an ensemble machine learning thin plate spline 
#' interp.rast<-machisplin.mltps(int.values=Mydata, covar.ras=raster_stack)
#' 
#' machisplin.write.geotiff(mltps.in=interp.rast)
...
machisplin.write.geotiff<-function(mltps.in, out.names= NULL, overwrite=TRUE){
	...
	#write summary table
	#write summary table
	file_outCSV<-paste0("MACHISPLIN_results_",sample(100000:999999,1),".csv")
	write.csv(table.O,file_outCSV)
	...
	write(legend,file=file_outCSV,append=TRUE)
	write(legend0,file=file_outCSV,append=TRUE)
	write(legend1,file=file_outCSV,append=TRUE)
	write(legend2,file=file_outCSV,append=TRUE)
	write(legend3,file=file_outCSV,append=TRUE)
	write(legend4,file=file_outCSV,append=TRUE)
	write(legend5,file=file_outCSV,append=TRUE)
	write(legend6,file=file_outCSV,append=TRUE)
}
...
#' @param rast.in high-resolution raster brick at total extent (final) of all downscaling.  This should be the rast.in in used in the machisplin.tiles.create function.
#' @param int.values a data frame with the first two columns as coordinates of interpolated site named exactly as 'long' and 'lat', in this order, and after, any number of layers to be downscaled; each as a new column
#' @param out.ncol number of columns in the processing grid
#' @param out.nrow number of rows in the processing grid
#' @param feather.d  The number of pixels to feather the seams at end of analysis (in tool machisplin.merge.tiles). Default = 50 
#' @return This function splits the high-resolution raster brick into smaller tiles for downscaling using the machisplin.mltps function.  outputs performance values, the algorithm(s) used, and rasters for use in GIS output from the 'machisplin.mltps' function
#' @export
#' @import terra
#' @examples
#' library(MACHISPLIN)
...
machisplin.tiles.create<-function(rast.in, int.values, out.ncol=3, out.nrow=3, feather.d=50){
#rast.in=ALT;out.ncol=3; out.nrow=3;feather.d=50; int.values=Mydata
print(paste("                                         ",Sys.time()))
#specify total extents
l <- list()
feather.d<-(feather.d/2)
totalExt<-terra::ext(rast.in)
nRx<-out.nrow
nCx<-out.ncol
nr.in<-nrow(rast.in)
nc.in<-ncol(rast.in)
total_long<-((totalExt[2]-totalExt[1]))
total_lat<-((totalExt[4]-totalExt[3]))
long_pix<-total_long/nc.in
lat_pix<-total_lat/nr.in
#specify lat distance
longDist<-((totalExt[2]-totalExt[1])/nCx)
#specify long distance
latDist<-((totalExt[4]-totalExt[3])/nRx)
#sampling grid 
m2<-0;m1<-0;new.df<-NULL;new.df2<-NULL; rast.out<-NULL;ext.out<-NULL;dat.out<-NULL
print("###########################################################################################")
print("###########################################################################################")
print("#####################       Preparing sampling grids          #############################")
print("###########################################################################################")
print("###########################################################################################")
...
for (j in 1:nRx){
    for (h in 1:nCx){
		m1<-m1+1
		new.df[[m1]]<-c((totalExt[1]+((longDist*(h-1))-(long_pix*feather.d))),(totalExt[1]+((longDist*h)+(long_pix*feather.d))),(totalExt[3]+((latDist*(j-1)))-(lat_pix*feather.d)),(totalExt[3]+((latDist*j))+(lat_pix*feather.d)))
	}
}
...
print("###########################################################################################")
print("###########################################################################################")
print("#####################         Preparing raster tiles          #############################")
print("###########################################################################################")
print("###########################################################################################")
for (h in 1:length(new.df)){
	ext.out[[h]] <- terra::ext(new.df[[h]][1], new.df[[h]][2], new.df[[h]][3], new.df[[h]][4])
	rast.out [[h]] <- terra::crop(rast.in, ext.out[[h]])
	}

for (h in 1:length(new.df)){
	mid.x<- (new.df[[h]][1]+new.df[[h]][2])/2
    mid.y<- (new.df[[h]][3]+new.df[[h]][4])/2	
	if (h==1) {
	my_polygon <- as.polygons(ext.out[[1]], crs = "+proj=longlat +datum=WGS84")
	my_points<-data.frame(cbind(mid.x,mid.y,1))
	}else{
	my_polygon2 <- as.polygons(ext.out[[h]], crs = "+proj=longlat +datum=WGS84")
	my_polygon<-terra::union(my_polygon,my_polygon2)
	my_points[h, ] <-cbind(mid.x,mid.y,h)}	
	}
print(paste("                                         ",Sys.time()))
print("###########################################################################################")
print("###########################################################################################")
print("##############         Preparing input datasets for each tile          ####################")
print("###########################################################################################")
print("###########################################################################################")
...
#specify coordinate system
#proj4string(MyShp) <- CRS("+proj=longlat +datum=WGS84 +ellps=WGS84")
#MyShp <- crs("+proj=longlat +datum=WGS84 +ellps=WGS84")
#######################
## convert SpatialPointsDataFrame to Terra SpatVector
terra_points <- vect(MyShp)
#make list to loop over 
#the column 3 to 4 in temp_dat are the temperature variable 
#and the column 5 to xx are the topographic variables 
#str(dat_tps)
for (h in 1:length(new.df)){
	ext.out[[h]] <- terra::ext(new.df[[h]][1], new.df[[h]][2], new.df[[h]][3], new.df[[h]][4])
	dat.out [[h]]	<- as.data.frame(terra::crop(terra_points, ext.out[[h]]))	 
	}
...
##################################################################################################
##################################################################################################
#' Show diagram of sub-sampling grid
#' @param rast.in output from machisplin.tiles.create tool  
#' @return This function displays the order of analysis - rasters input into machisplin.tiles.merge rast.in parameter MUST match this order.  
#' @export
#' @import terra
#' @examples
#' library(MACHISPLIN)
...
machisplin.tiles.id<-function(rast.in){
plot(rast.in$e.poly)
#points(rast.in$ext3$mid.x,rast.in$ext3$mid.y, labels =rast.in$ext3$V3)
text(rast.in$e.ID$mid.x,rast.in$e.ID$mid.y, label = rast.in$e.ID$V3, col = "red", cex = 3)}

 
##################################################################################################
##################################################################################################
#' Merge multiple MACHISPLIN models
#' @param rast.in a raster stack of high-resolution downscaled tited rasters output from the machisplin.mltps function 
#' @param rast.full.ext a high-resolution raster brick at total extent (final) of all downscaling.  This should be the rast.in in used in the machisplin.tiles.create function.
#' @param out.ncol number of columns in the processing grid
#' @param out.nrow number of rows in the processing grid
#' @return This function merges the high-resolution downscaled layers tiles output from  the machisplin.mltps function.  o
#' @export
#' @import terra
#' @examples
#' ########################################EXAMPLE 1################################################
...
machisplin.tiles.merge<-function(rast.in, rast.full.ext, in.ncol=2, in.nrow=3){
	feath.ras.out<-NULL; nCx=in.ncol; nRx = in.nrow
	l<-list()
	g.clock=0
	print("Feathering tiled surfaces ")
	print(paste("Feathering",nRx,"rows and",nCx,"columns"))
	print(paste("                                         ",Sys.time()))
	for (j in 1:nRx){
	  for (h in 1:nCx){
		...
		print(paste("Seam is at edge of",v,"and",v+1,"tiles"))
		print(paste("                                         ",Sys.time()))
		AAA<-terra::extend(rast.in[[v]],rast.full.ext)+terra::extend(rast.in[[v+1]],rast.full.ext)
		MyShp<-terra::as.points(AAA, value=TRUE)		
		#specify coordinate system
		terra::crs(MyShp) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
		#
		blend.ext<-terra::ext(MyShp)
		#crop
		bR1 <- terra::crop(rast.in[[v]], blend.ext)
		bR2 <- terra::crop(rast.in[[v+1]], blend.ext)
		bR2 <- resample(bR2, bR1, method = "near")
		##
		x_bR1<- terra::xFromCell(bR1, 1:terra::ncell(bR1))
		x_bR2<- terra::xFromCell(bR2, 1:terra::ncell(bR2))
		...
		we.R.d2 <-bR2 * we.R.d2a
		feath.ras<-we.R.d2+we.R.d1
		feath.ras<- terra::extend(feath.ras,rast.full.ext)
		terra::plot(feath.ras)
		if(is.null(feath.ras.out)==TRUE){
			feath.ras.out<-feath.ras
		}else {
			feath.ras.out<-c(feath.ras.out,feath.ras)
			}
		#r_list <<-list(feath.ras, feath.ras.out)
		#feath.ras.out<<- terra::rast(r_list)
		}
		}
		}
		gc()
	...
	for (j in 1:nRx){
	  for (h in 1:nCx){
		...
		gc()
		raster_sTPS<-rast.in
		AAA<-terra::extend(raster_sTPS[[v]],rast.full.ext)+terra::extend(raster_sTPS[[g]],rast.full.ext)
		MyShp<-terra::as.points(AAA, value=TRUE)
		terra::crs(MyShp) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
		#
		blend.ext<-terra::ext(MyShp)
		...
		TEMP12<<-feath.ras.out
	print("Finished feathering horizontal edges")
	print("Merging all raster tiles into single raster")
	if(nRx*nCx>2){
		for (j in 1:terra::nlyr(feath.ras.out)) {
			    if(j==1){
					ic<-terra::sprc(feath.ras.out[[1]])
				}else{
					ic<-terra::sprc(feath.ras.out[[j]],ic)
		}}
		rast.mosaic.feath <- terra::mosaic(ic, fun= "mean")
		for (j in 1:length(rast.in)) {
			    if(j==1){
					ig<-terra::sprc(rast.in[[1]])
					#ig<-rast.in[[1]]
				}else{
					#r_list <- list(ig,rast.in[[j]])
					#ig<- terra::rast(r_list)
					ig<-terra::sprc(rast.in[[j]],ig)
		}}
		rast.mosaic.in <- terra::mosaic(ig, fun= "mean")
		etal<-terra::sprc(rast.mosaic.feath,rast.mosaic.in)
		final.TPS<- terra::mosaic(etal, fun= "first")
		}
	if(nRx*nCx==2){
		final.TPS<-terra::merge(feath.ras.out,rast.mosaic.in)}
	if(nRx*nCx==1){
		final.TPS<-rast.mosaic.in
		}
l<-final.TPS
}
...
# includes estimates of the standard error of the fitted values added 2nd may 2005
	...
	for (i in 1:n.folds) {
		...
		if (family == "bernoulli") {
			cv.roc.stats[i] <- machisplin.roc(y_i,u_i)
			cv.calibration.stats[i,] <- machisplin.calibration(y_i,u_i,"binomial")
			threshold.stats[i] <- approx(ppoints(u_i), sort(u_i,decreasing = T), prevalence)$y
		}

		if (family == "poisson") {
			cv.calibration.stats[i,] <- machisplin.calibration(y_i,u_i,"poisson")
		}
	}
...
#extract fitted values and summary table
	...
	if (family == "bernoulli") {  # do this manually as we need the residuals
		...
		self.roc <- machisplin.roc(y_i,u_i)
		self.calibration <- machisplin.calibration(y_i,u_i,"binomial")
	}
	...
	if (family == "poisson") {   # do this manually as we need the residuals
		...
		residuals <- sqrt(abs(deviance.contribs * 2))
		residuals <- ifelse((y_i - u_i) < 0, 0 - residuals, residuals)
		self.calibration <- machisplin.calibration(y_i,u_i,"poisson")
	}
...
machisplin.roc <-function (obsdat, preddat) {
...
machisplin.calibration <- function(obs, preds, family = "binomial")  {
#
